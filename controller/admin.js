...
global.suggestionBox = function(startDate, endDate){
    return new Promise(resolve => {
        db.query("SELECT * FROM suggestion WHERE suggestion_date BETWEEN ? and ?",[startDate+" 00:00:00", endDate+" 23:59:59"], function(err,result){
            resolve(result)
        })
    })
}
...
exports.getExport = async function(req,res){
    if (req.session.loggedin==true){
        ...
        var suggestion = await suggestionBox(datefrom, dateto);
        var isifile = [];
        if(type!="suggestion"){
            ...
            for (let i = 0; i < suggestion.length; i++) {
                var userData = await getUserById(suggestion[i].user_id)
                isifile.push([
                    userData[0].Email,
                    suggestion[i].mind,
                    suggestion[i].wish,
                    moment(suggestion[i].suggestion_date).format("YYYY-MM-DD")
                ])
            }
